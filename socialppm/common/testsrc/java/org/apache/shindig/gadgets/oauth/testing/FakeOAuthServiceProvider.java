...
public class FakeOAuthServiceProvider implements HttpFetcher {
  ...
  private MessageInfo parseMessage(HttpRequest request) {
    ...
    if (validParamLocations.contains(OAuthParamLocation.AUTH_HEADER)) {
      ...
      if (aznHeader != null) {
        ...
        for (Parameter p : OAuthMessage.decodeAuthorization(aznHeader)) {
  ...
  private static class ParsedUrl {
    ...
    String query = null;
    List<Parameter> decodedQuery = null;
    ...
    public List<Parameter> getParsedQuery() {
    ...
    public String getQueryParam(String name) {
      for (Parameter p : getParsedQuery()) {
  ...
  private HttpResponse handleAccessTokenUrl(HttpRequest request)
    ...
    List<Parameter> params = OAuth.newList(
        "oauth_token", accessToken,
        "oauth_token_secret", accessTokenSecret);
    if (sessionExtension) {
      params.add(new Parameter("oauth_session_handle", state.sessionHandle));
      if (reportExpirationTimes) {
        params.add(new Parameter("oauth_expires_in", "" + TOKEN_EXPIRATION_SECONDS));
      }
    }
    if (returnAccessTokenData) {
      params.add(new Parameter("userid", "userid value"));
      params.add(new Parameter("xoauth_stuff", "xoauth_stuff value"));
      params.add(new Parameter("oauth_stuff", "oauth_stuff value"));
    }
